...
impl Upgradable for DiscordRoles {
    fn get_staging_duration(&self) -> WrappedDuration {
        self.upgrade.staging_duration.into()
    }

    fn stage_code(&mut self, code: Vec<u8>, timestamp: Timestamp) {
        self.assert_owner();
        assert!(
            env::block_timestamp() + self.upgrade.staging_duration < timestamp,
        ...
        self.upgrade.staging_timestamp = timestamp;
    }

    fn deploy_code(&mut self) -> Promise {
        if self.upgrade.staging_timestamp < env::block_timestamp() {
            env::panic(
                &format!(
                    "Deploy code too early: staging ends on {}",
                    self.upgrade.staging_timestamp + self.upgrade.staging_duration
                )
                .into_bytes(),
            );
        }
